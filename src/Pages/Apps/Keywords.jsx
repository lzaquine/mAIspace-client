...
function Keywords() {
  ...
  const getKeywords = async () => {
    try {
      let response = await axios.get(`${process.env.REACT_APP_API_URL}/app/keywords`);
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="text" name="question" className='input input-bordered w-full rounded-lg max-w-xs mt-1 mb-3 mt-8 text-center text-neutral-400 opacity-95 justify-center' placeholder="Text" onChange={(e) => setQuestion(e.target.value)} value={question} /><br/>
