...
function Marvbot() {
  ...
  const getMarvbot = async () => {
    try {
      let response = await axios.get(`${process.env.REACT_APP_API_URL}/app/marvbot`);
...
const token = localStorage.getItem('authToken');
  ...
  return (
    <div>
      <img src={robot} alt="logo" className='card w-24 rounded-lg left-1/2 transform -translate-x-1/2 mt-7 mb-5  saturate-200 glass justify-center'/>

      <h1>Marv</h1>

      {/* <h2>{prompt}</h2> */}
      <h2 className='mt-2'>Marv is a factual chatbot that is also sarcastic and rude.</h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="text" name="question" className='input input-bordered w-full rounded-lg max-w-xs mt-1 mb-3 mt-8 text-center text-neutral-400 opacity-95 justify-center' placeholder="Who are you?" onChange={(e) => setQuestion(e.target.value)} value={question} /><br/>
